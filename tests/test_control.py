...
class TestAdmission:
    def test_a_rule_rejected_write_reaches_no_region(self) -> None:
...
class TestSetRegister:
    def test_an_accepted_premise_write_returns_written(self) -> None:
        control = make_control()
        result = control.set_premise(
            "window", "w", expected_version=0, writer="operator"
        )
...
class TestManyWritersAtOnce:
    def test_concurrent_writes_take_every_sequence_once(self) -> None:
        control = make_control()
        ...
        for thread in threads:
            thread.join()
        sequences = [c.sequence for c in control.reader.read_board()]
...
class TestWhoWrites:
    def test_a_writer_that_never_registered_may_still_write(self) -> None:
        ...
        # an agent, and the record carries it under that name.
        assert control.write("application", "x", writer="an-operator") == Written(
            sequence=1
        )
        (contribution,) = control.reader.read_level("application")
        assert contribution.writer == "an-operator"
