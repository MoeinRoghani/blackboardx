...
| Term | Meaning |
| --- | --- |
| **`BoardStore`** | The protocol a store implements: nineteen methods. Three write the record, four read it, one removes a board, five hold the run, being its two deadlines and its outcome, four hold the agents of a run and how far each has got, and two hold the notifications a write recorded and nothing has sent. Every one names a board first. |
...
| **Reach** | The transport a `Control` is given for the addresses on the run. It takes an address and a notification and sends one to the other, on the calling thread. `HttpNotifier.reach` is one. A process given none reaches the agents it holds callables for and no others. |
| **Lane** | One agent's queue and the worker that drains it. `HttpNotifier.to` opens one and returns it, which is what lets agents be reached at the same time. It is the `notify` callable an `Agent` takes, and closing one releases its worker without closing the notifier or any other lane, which is how a finished run gives back its threads. Calling it queues and returns, so a lane whose notifier was given the store clears the outbox row itself, after the send. |
...
A premise, its content, and the content it began with are three different
things.
...
A premise's value is what that premise says right now. Its opening value is
what it said when the run began.

What a premise holds is never called a fact. Premise and conclusion are a pair
on one axis, given against concluded, and "fact" does not name either end of
it: a conclusion an agent reaches is a fact too, so the word does not separate
the two kinds of region.

## One distinction this project keeps

**Complete, and settled.** The termination predicate answers `COMPLETE` about the result: the application has what it needs. `Settled` describes the run: nothing happened for the idle limit. A run given no predicate settles without anything being asked, and a predicate answers complete on a run that then expires on the wall clock.
