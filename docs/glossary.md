...
| **Lane** | One agent's queue and the worker that drains it. `HttpNotifier.to` opens one and returns it, which is what lets agents be reached at the same time. It is the `notify` callable an `Agent` takes, and closing one releases its worker without closing the notifier or any other lane, which is how a finished run gives back its threads. |
