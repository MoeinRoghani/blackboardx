...
| **Tool** | One read or write of the agent surface, in the form a language model API accepts: a name, a description, and a schema for what may be sent. A model answers with a request to call one, and the application runs it. `ToolDescriptor` in the code. |
