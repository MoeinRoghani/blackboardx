...
Every event is recorded in the order it occurred: opening premise values, accepted and rejected writes, dispatches, acknowledgments, and the closing state. Events that reached the board carry their sequence number; a rejected write never reached it and carries none.
